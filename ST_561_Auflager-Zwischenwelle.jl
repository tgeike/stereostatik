...
## Auflagerreaktionen in der Zwischenwelle eines Getriebes

Es sollen die Lagerreaktionen bei einer Zwischenwelle eines Getriebes bestimmt werden. Lager A ist ein Loslager, Lager B ein Festlager. Die Zahnkräfte in axialer, tangentialer und radialer Richtung sind für beide schrägverzahnten Stirnräder bekannt.

Zuerst laden wir die benötigten Pakete.
"""

# ╔═╡ 3e261363-0665-4535-b5df-2d39d7bbabd6
md"""
### Schritt 1: Alle Parameter zusammentragen"""

# ╔═╡ f2e8468d-304c-46c3-8271-e8091a620f11
md"""Alle Parameter sollen in einer `struct` zusammengefasst werden. Als Kommentar ist jeweils die unterstellte Einheit angegeben. Hier wird vorerst die Struktur definiert. Die Befüllung mit den konkreten Zahlenwerten erfolgt im nächsten Schritt."""
...
param = parameter(a=70.0,b=100.0,c=50.0,r2=50.47,r3=40.62,Ft2=2602,Fa2=697,Fr2=980,Ft3=3233,Fa3=570,Fr3=1195)
...
function ggb(u,p)
	...
	FB = u[3:5]
	Mxres = u[6]
	F2 = [p.Fa2;p.Fr2;-p.Ft2]
	F3 = [-p.Fa3;-p.Fr3;-p.Ft3]
	Fres = FA + FB + F2 + F3
	Mres = cross([-(p.a+p.b);-p.r2;0.0],F2) + cross([-p.a;p.r3;0.0],F3) + 
	cross([-(p.a+p.b+p.c);0;0],FB) + [Mxres;0.0;0.0]
	res = [Fres;Mres]
	return res
end
...
# ╔═╡ b96f6394-730b-466b-9274-102915c0bb61
u0 = zeros(6);

# ╔═╡ e629d3ab-9c6b-4e3d-a06a-129e3f589f4a
prob = NonlinearProblem(ggb, u0, param);

# ╔═╡ a2bbe7b2-23e5-4f7f-8795-59fadbba37b3
md"""Mit dem Befehl `solve` wird das Gleichungssystem gelöst."""

# ╔═╡ 10e2d2b2-eba3-4f7a-abd1-9f62ee75a381
sol = solve(prob);

# ╔═╡ bca7a6f0-beba-43d3-af80-c0e6fe27fb19
md"""Die Lösung ist wie folgt:"""

# ╔═╡ 5002d16e-5bd4-4aa9-aa5c-fdd37af13d3f
sol.u

# ╔═╡ 7c89e55f-af73-4c23-91a4-e3c4e55cdbec
md"""
Zur Erinnerung: Die gesuchten Größen sind in der Spaltenmatrix ``\mathbf{u}`` wie folgt angeordnet.
```math
\mathbf{u} = \left(F_{\mathrm{A}y},F_{\mathrm{A}z},F_{\mathrm{B}x},F_{\mathrm{B}y},F_{\mathrm{B}z},M_{x\mathrm{res}}\right)^T\;.
```
"""

# ╔═╡ 51890648-f42f-4956-a4a0-9cd2761a8c6a
md"""Wir überzeugen uns durch Einsetzen, dass die gefundene Lösung tatsächlich die Gleichgewichtsbedingungen erfüllt."""

# ╔═╡ d2122e6a-9cd2-4364-b28a-3a454b6d1580
ggb(sol.u,param)

# ╔═╡ fe6af8a8-3216-4227-8404-c793fe43756f
md"""
### Schritt 4: Post Processing
Aus den gegebenen Werten werden die Radialkräfte und Axialkräfte in den Lagern und das Drehmoment am Abtrieb berechnet. Alle Kräfte sind in der Einheit N angegeben."""

# ╔═╡ d90d84e6-9b97-4a43-8141-b0cf5f2d63b4
md"""Radialkräfte in den Lagern A und B"""

# ╔═╡ cbfcb648-9281-4f3d-9e8c-74f50cd55711
FAr = sqrt(sol[1]^2 + sol[2]^2)

# ╔═╡ 35905600-8622-475b-b366-ee0d31cb38e2
FBr = sqrt(sol[4]^2 + sol[5]^2)

# ╔═╡ 941edb45-458f-4bf0-8f98-345064b13f46
md"""Axialkraft im Lager B"""

# ╔═╡ d89a48fb-c861-4f85-8094-45b29c813d46
FBa = sol[3]

# ╔═╡ f91beb0a-8ee3-49f1-8a62-2f6acd082e64
md"""**Antwort** Die Radialkraft in Lager A beträgt $(round(FAr/1000,digits=2)) kN und in Lager B  $(round(FBr/1000,digits=2)) kN. Die Axialkraft im Lager B beträgt  $(round(abs(FBa)/1000,digits=2)) kN."""

# ╔═╡ 366fbdd0-d71a-4bb8-8ce7-b82caa77f46b
md"""
**Nachbemerkung** Das resultierende Drehmoment müsste im statischen Fall Null sein. Andernfalls führen die gegebenen Zahnkräfte zu einer Beschleunigung der Zwischenwelle.
Das resultierende Drehmoment beträgt $(round(sol[6]/1000,digits=4)) N m. Das Drehmoment, das an Zahnrad 2 eingeleitet wird, beträgt $(round(param.Ft2*param.r2/1000,digits=1)) N m. Das resultierende Drehmoment ist demnach praktisch Null. Die kleine Abweichung von Null resultiert aus der Rundung bei der Angabe der Zahnkraftwerte."""

# ╔═╡ c1fb269d-8871-48d3-b5cf-aa9c3b3bdead
sol[6]/(param.Ft2*param.r2)

# ╔═╡ dc3f649a-ee9d-4fcc-a952-62621be10a53
md"""
### Alternativer Lösungsweg zu Schritt 3"""

# ╔═╡ 0e9b12e6-eeb9-4f0a-a532-8f7bd6e173a6
begin
	A = zeros(6,6)
	b = -ggb(u0,param)
	for j = 1:6
...
# ╔═╡ e49ec358-2cbd-43bc-b6f5-737a6fbcb6d6
md""" Wenig überraschend ergibt sich die gleiche Lösung für die sechs gesuchten Größen wie beim Lösen mit dem nichtlinearen Löser."""

# ╔═╡ e8c1130e-8950-4cda-851b-7a171504ea04
md"""
### Anhang"""

# ╔═╡ a9c47c6f-c5e6-4ca4-b8d1-b558a2cc8208
md"""ähnlich zur Aufgabe 7.1 aus Sauer (Hrsg.): *Konstruktionselemente des Maschinenbaus - Übungsbuch*, Springer, 2011, Seite 102ff"""
...
manifest_format = "2.0"
project_hash = "cd89668036cc716c24975657931f5ae1c06e2202"
