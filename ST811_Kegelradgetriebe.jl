...
using LinearAlgebra,NonlinearSolve,Plots
...
md"""Da alle Längen in mm und alle Kräfte in N angegeben sind, werden die Ergebnisse für alle Kräfte ebenfalls in N herauskommen. Das gesuchte Drehmoment wird in N mm herauskommen. Für die finale Angabe in der üblichen Einheit N m ist demnach am Ende noch durch 1000 zu dividieren."""
...
function ggb(u,p)
	...
	FZ = [p.Fa;-p.Fr;p.Ft] # Zahnkraft
	Fres = FA + FB + FZ # Kräftegleichgewicht
...
# ╔═╡ ae32c6f1-9f1a-4dd8-ae38-9b27cc3b00e9
md"""Zur Erinnerung: Die gesuchten Größen sind in der Spaltenmatrix ``\mathbf{u}`` wie folgt angeordnet.
```math
\mathbf{u} = \left(F_{\mathrm{A}y},F_{\mathrm{A}z},F_{\mathrm{B}x},F_{\mathrm{B}y},F_{\mathrm{B}z},M_{\mathrm{Ab}}\right)^T\;.
```
"""
...
md"""Radialkräfte in den Lagern A und B (in N)"""
...
md"""Axialkraft im Lager B (in N)"""

# ╔═╡ d89a48fb-c861-4f85-8094-45b29c813d46
FBa = sol[3]

# ╔═╡ 748794d4-e996-45ca-8c25-f20354d965ea
md"""Drehmoment am Abtrieb (in N m)"""

# ╔═╡ ab0a8c5e-fb28-4d6f-a36a-c725b1a63bab
Mab = sol[6]/1000
...
```
mit Koeffizientenmatrix ``\mathbf{A}`` und rechter Seite ``\mathbf{b}`` extrahiert. Dazu muss die Funktion `ggb` sieben Mal aufgerufen, ein Mal mit ``\mathbf{u}=\mathbf{0}`` (das liefert ``-\mathbf{b}``) und dann sechs Mal, wobei jeweils eine Position in ``\mathbf{u}`` zu 1 und alle anderen zu 0 gesetzt werden (das liefert nacheinander die 6 Spalten von ``\mathbf{A}``). Am Ende wird mit der Codezeile `A\b` das lineare Gleichungssystem gelöst. Wenig überraschend ergibt sich die gleiche Lösung für die sechs gesuchten Größen.
"""
...
FZ = [param.Fa;-param.Fr;param.Ft]

# ╔═╡ b29cbb48-76d1-47a1-b735-fa6cb4466d7f
M2(x) = M1(x) - cross([-x+param.a;param.r;0],FZ);

# ╔═╡ 91484d74-bf19-4118-a609-3f97fc27b972
M(x) = x < param.a ? M1(x) : M2(x);

# ╔═╡ deb8213c-3b6d-4789-88f3-ca5264360673
plot(0:param.a+param.b,[x->M(x)[1]*1e-3;x->M(x)[2]*1e-3;x->M(x)[3]*1e-3],lw=3,label=["Mx" "My" "Mz"])
...
md"""
### Anhang
ähnlich zur Aufgabe 15.10 aus Decker Aufgabensammlung
"""
...
manifest_format = "2.0"
project_hash = "5ea38bdab261c05b342d2cadbe81f71af84aabb6"
