...
using ForwardDiff,Plots,LinearAlgebra,NonlinearSolve

# ╔═╡ 1f0fa310-429a-11ef-3e18-5bb86f4cb3e7
md"""
# Technische Berechnungen mit Julia
## Auflagerreaktionen bei einer Seilwinde

siehe auch Szabo Einführung S. 58f
"""
...
function ggb(U,p)
	...
	FA = U[1:3]
	FB = [0; U[4:5]]
	FP = [0;U[6]*cos(ϕ);-U[6]*sin(ϕ)]
	#println(FA,FB,FP)
	FG = [0;0;-FG]
	Fres = FA+FB+FG+FP
	Mres = cross([a;-r;0],FG) + cross([l;0;0],FB) + cross([l+c;h*sin(ϕ);h*cos(ϕ)],FP)
	#println(Mres)
...
# ╔═╡ 821e24be-0dad-43f7-99df-cba46cb00204
p = 0.0

# ╔═╡ f178f49e-e3f5-48de-8e7d-08d8f268672a
V = [1;2;3;4;5;6]

# ╔═╡ b7c586f1-103b-42c0-aac4-7a5ecaf54064
ggb(V,p)
...
prob = NonlinearProblem(ggb, U0, p)

# ╔═╡ 85c2ece5-26e8-4845-8f0f-9fada854bf77
ggb(U0,p)
...
FAabs = sqrt(sol[2]^2 + sol[3]^2)

# ╔═╡ 35905600-8622-475b-b366-ee0d31cb38e2
FBabs = sqrt(sol[4]^2 + sol[5]^2)
...
manifest_format = "2.0"
project_hash = "4b85e5e44f8b5dc1061b7d7a3fa3ba6c0ac893f8"
